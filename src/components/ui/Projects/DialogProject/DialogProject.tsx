...
export function DialogProject({
  ...
  return (
    <dialog
      ref={dialogRef}
      className="backdrop:bg-black/50  bg-blu-100 max-w-[80%] max-h-full lg:w-1/3 p-4 shadow-2xl outline-none flex flex-col gap-4 project-dialog text-grayish "
    >
      <div className="flex justify-between items-center border-b border-grayish py-4">
        <p className="text-yel-100 font-styled tracking-widest text-xl lg:text-3xl leading-none">
      ...
      <img
        ...
        alt={`cover of ${selectedProject?.name} project`}
        className="w-96 lg:mx-auto"
      />
      <article className="text-xs lg:text-sm">
        <p className="font-styled-bold text-lg text-yel-100 lg:text-2xl">
          About
        </p>
        <p className="text-justify text-grayish">
          {selectedProject?.description}
        </p>
      </article>
      <ul className="flex lowercase text-xs gap-2 justify-between lg:justify-start lg:gap-6 flex-wrap">
        {selectedProject?.techStackList.map((tech, index) => (
          <LittleTechStackItem key={index} {...tech} />
        ))}
      </ul>
      <ul
